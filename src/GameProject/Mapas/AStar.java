...
public class AStar {
	...
	public static List<Node> findPath(Mapas mapa, Vector2i start, Vector2i end){
		...
		while(openList.size() > 0) {
			...
			for(int i = 0; i < 9; i++) {
				...
				int xi = (i%3) - 1;
				int yi = (i/3) - 1;
				Tile tile = Mapas.tiles[x+xi+((y+yi)*Mapas.WIDTH)];
				if(tile == null) continue;
				if(tile instanceof WallTile) continue;
				if(i == 0) {
					Tile test = Mapas.tiles[x+xi+1+((y+yi) * Mapas.WIDTH)];
					Tile test2 = Mapas.tiles[x+xi+((y+yi+1) * Mapas.WIDTH)];
					...
					Tile test = Mapas.tiles[x+xi+((y+yi-1) * Mapas.WIDTH)];
					Tile test2 = Mapas.tiles[x+xi+1+((y+yi) * Mapas.WIDTH)];
						if(test instanceof WallTile || test2 instanceof WallTile) {
							continue;
						}
				}
